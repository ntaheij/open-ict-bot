...
const { auditChannel, auditWebhook } = require("../../configs/config.json");
...
async function send_log(c, guild, color, title, description, thumb) {
  try {
    ...
    const LogEmbed = new Discord.EmbedBuilder()
      ...
      .setFooter({
        text: guild.name + " | Logger Module",
        iconURL: guild.iconURL({ format: "png" })
      }
      );

    const logger = await c.channels.cache.get(auditChannel);
    if (!logger) throw new SyntaxError("CHANNEL NOT FOUND");

    try {
      logger.send({ embeds: [LogEmbed] })
    } catch {
      return;
    }
